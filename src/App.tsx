...
const blogPosts: BlogPost[] = [
  ...
  },
  {
    ...
    sections: [
      ...
      { title: '接口复用与页面恢复', body: '新客户端的核心不是一次性复刻全部旧功能，而是先封装登录态、请求客户端、返回模型和公共 UI 工具，再按业务优先级恢复页面。这样旧接口仍然可以作为业务能力来源，但公开展示时只讲协议复用和模块边界，不暴露真实服务地址、测试账号或部署脚本。' },
      ...
      { title: '公开展示取舍', body: '这个案例适合展示历史系统整理能力、移动端兼容策略和阶段验收方法，不适合公开真实服务器、账号、签名文件、发布包哈希或完整接口清单。站点内容只保留抽象后的模块地图、重构思路和脱敏运行证据。' },
    ],
...
const caseStudies: CaseStudy[] = [
  ...
  },
  {
    ...
    evidence: ['64 位客户端模块地图', '服务端接口复用边界', '阶段迁移与验收文档', '脱敏运行截图待补充'],
    architecture: [
