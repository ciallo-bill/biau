...
> = {
  ...
  'ozon-erp': {
    ...
    note: '生产普通注册当前默认开放，显式关闭开关仍可关闭；登录页主品牌和后台侧栏已接入 BIAU Port / 泊岸外壳，状态页只检查公开入口是否响应。',
  },
  'biau-playlab': {
  ...
  'pet-workspace': {
    ...
    note: '展示页已接入 BIAU Port / 泊岸标题与 favicon；入口可达代表展示页和截图可访问，正式 APK 下载仍需签名包、校验摘要和人工确认。',
  },
}
...
export const reliabilityProjects: ReliabilityProject[] = [
  {
    ...
    nextActions: ['把 `main-site:synthetic` 接入定时任务或内部监控。', 'Cloudflare Pages Functions 部署后配置 `ASSISTANT_MODEL_*` 并复查公开助手状态。'],
  },
  {
    id: 'legal-rag',
  ...
  },
  {
    ...
    checks: [
      {
        ...
        label: '演示入口',
        status: 'unchecked',
        description: '检查 ERP 登录/演示入口是否响应，并确认入口以 BIAU Port / 泊岸品牌外壳承载 Ozon ERP 产品名。',
        evidence: '由 L0 入口检测自动合并；登录页主视觉和登录后侧栏已改为泊岸统一图标与归属说明。',
        cadence: '每次生成状态页',
        ownerHint: '公开入口',
      ...
      },
      {
        ...
        label: '登录 / 注册策略',
        status: 'planned',
        description: '验证登录可用、生产普通注册默认开放、显式关闭开关可关闭、新用户默认角色安全。',
        evidence: 'ERP 仓库注册逻辑已改为生产默认开放普通注册，显式关闭值仍会关闭；新注册账号默认是 operator。',
        cadence: '鉴权改动后',
    ...
    gates: ['生产普通注册已改为默认开放；滥用防护、公开演示账号和实际线上重启仍需独立观察。'],
    nextActions: [
      ...
      '复查线上重启后登录页是否显示注册切换，并观察新账号默认 operator 权限。',
    ],
  },
  {
    id: 'xunqiu',
    ...
    checks: [
      {
        ...
        label: '产品展示页',
        status: 'unchecked',
        description: '检查寻球展示页、文档和阶段包入口是否响应，并确认标题/favicon 归属泊岸展示体系。',
        evidence: '由 L0 入口检测自动合并；展示站首页、文档页和 404 页已补 BIAU Port / 泊岸标题与 favicon。',
        cadence: '每次生成状态页',
        ownerHint: '公开入口',
    ...
    checks: [
      {
        ...
        evidence: '展示页已补 BIAU Port / 泊岸标题与 favicon；未确认公开签名包前不放真实下载。',
        cadence: '展示页更新后',
    ...
    checks: [
      {
        ...
        label: '游戏站入口',
        status: 'unchecked',
        description: '检查 Playlab 静态站入口是否响应，并确认 BIAU Playlab 命名、泊岸 favicon、导航副标题和页脚归属已同步。',
        evidence: '由 L0 入口检测自动合并；游戏站已统一 BIAU Playlab 命名并换用 BIAU Port / 泊岸 favicon，导航和页脚也显示泊岸归属。',
        cadence: '每次生成状态页',
        ownerHint: '公开入口',
